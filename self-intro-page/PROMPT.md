...
5. **質問は全部まとめて1回で聞く（1問ずつ順番に聞かない・最重要）。** 下の記入テンプレを1回で提示し、私が全項目をまとめて1回で答える。私の答えを自分で書いたり代筆して勝手に進めない（`user…` のように相手の発言を作るのは厳禁）。空欄は空欄のままにし、勝手に埋めない。
...
### 2. 記入テンプレを1回で渡す
下の記入テンプレを**1回でまとめて**提示し、私が1回でまとめて答えます（1問ずつ順番に聞かない）。任意（⑦・⑪）はスキップOK。
...
各項目の使われ方: ①名前・ふりがな→ヒーロー ②所属→赤枠チップ ③拠点→グレー枠チップ ④略歴→タイムライン(3〜6個) ⑤強み3つ→番号カード ⑥趣味→アイコン付きタグ ⑦SNS(任意)→SNSリンク(無ければ省略) ⑧座右の銘→引用ボックス ⑨ひとこと→メッセージ枠 ⑩色→色テーマ ⑪画像(任意)→アバター(無ければ頭文字)。

受け取ったら、その内容でページを作る。足りない項目があっても、まず今ある内容で作る（空欄は省略/仮にし、勝手に埋めない）。必要なら作った後に、足りない所だけまとめて1回だけ確認する。⑪画像は self-intro フォルダに入れたファイル名を使い、無ければ頭文字アバターで完成させる（画像は自分で生成しない）。
...
### 5.5 提出フォームを案内する（Levelaの課題提出）
公開が終わったら（＝公開URLができたら）、続けて提出フォームを案内する:
- 「課題として提出する場合は、下の提出フォームを開いて、いま公開したURLを『公開URL』欄に貼って送信してください」
- 提出フォームURL: https://docs.google.com/forms/d/e/1FAIpQLSdWsPAoTIosI2Y4VDs1mkRJ41kqx6cvJgluQrgNp5StilE8TA/viewform
- 設問: 名前／部署／提出する課題／公開URL／ひとこと。**送信は本人が行う（代理送信はしない）。**
